...
window.PSPRD = {render:function (container, data, vid, page, query, esc) {
  ...
  container.innerHTML='<div class="prd-reader"><header><h1>产品需求文档</h1><p>先阅读业务规则与验收；编号用于精确定位，控件技术信息按需展开。</p><div class="prd-tools"><label>页面 <select id="prd-page"><option value="">全部页面</option>'+pages.map(function(x){return '<option value="'+esc(x.id)+'">'+esc(x.title)+'</option>';}).join('')+'</select></label><label>搜索标题、正文或编号 <input id="prd-search" type="search"></label><button id="prd-export">导出完整 PRD（Markdown）</button></div><p id="prd-count" role="status"></p></header><div class="prd-layout"><nav id="prd-toc" aria-label="需求目录"></nav><div id="prd-sections"></div></div><dialog id="prd-edit-context"><h2>交给 Agent 精确修改</h2><p>复制下面的定位信息并补充修改意图；不会自动提交，也不会重新生成项目。</p><textarea rows="7" aria-label="Agent 修改上下文" readonly></textarea><button>关闭</button></dialog></div>';
  function draw() {
  ...
  container.onclick=function(event){
    var jump=event.target.closest('[data-jump]'); if(jump){var target=document.getElementById('prd-'+jump.dataset.jump); if(target) target.scrollIntoView({block:'start'});}
